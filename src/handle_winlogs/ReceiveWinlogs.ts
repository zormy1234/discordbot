...
export function parseTanksLine(line: string): ParsedLine | null {
...
export function parseShipsLine(line: string): ParsedLine | null {
  const trimmed = line.trim();
  if (!trimmed) return null;
  ...
  const parts = rest.split(/\s+/);
  if (parts.length < 3) return null; // rank, gid, username (minimum)
...
export default function handleWinlogs(client: Client) {
  client.on('messageCreate', async (message: Message) => {
    ...
    if (
      message.guild?.id === process.env.RC_WINLOGS_DISCORD &&
