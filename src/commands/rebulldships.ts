...
export async function execute(interaction: ChatInputCommandInteraction) {
  ...
  // Purge totals tables before rebuilding
  ...
  try {
    ...
    for (const r of rows as any[]) {
      ...
      const kd: number = r.deaths > 0 ? r.kills / r.deaths : r.kills;
      ...
      t.num_entries += 1;
      if (r.kills > 0) t.kd_values.push(kd);
      ...
      d.num_entries += 1;
      if (r.kills > 0) d.kd_values.push(kd);
      d.last_entry = r.created_at;
      d.recent_name = r.username;
      d.recent_clan_tag = r.clan_tag;

      if (r.kills + r.deaths > 0 && kd > d.highest_kd) d.highest_kd = kd;
      if (r.kills > d.highest_kills) d.highest_kills = r.kills;
      if (r.deaths > d.highest_deaths) d.highest_deaths = r.deaths;
    }
    ...
    for (const gid of Object.keys(totals)) {
      const t = totals[gid];
      const avg_kd =
        t.kd_values.length > 0 ? t.kd_values.reduce((a: number, b: number) => a + b, 0) / t.kd_values.length : 0;
    ...
    for (const key of Object.keys(daily)) {
      const d = daily[key];
      const avg_kd =
        d.kd_values.length > 0 ? d.kd_values.reduce((a: number, b: number) => a + b, 0) / d.kd_values.length : 0;
