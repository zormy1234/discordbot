import {
  ...
  export async function execute(interaction: ChatInputCommandInteraction) {
    ...
    async function fetchPage(page: number) {
      ...
      return rows as { gid: string; recent_name: string; recent_clan_tag: string | null; total_score: number }[];
    }
  
    async function buildContent(page: number) {
    ...
    collector.on('collect', async i => {
      ...
      if (i.customId === 'next' && page < MAX_PAGES - 1) page++;
