...
export async function execute(interaction: ChatInputCommandInteraction) {
  ...
  try {
    ...
    const [rows] = (await connection.execute<RowDataPacket[]>(
      `SELECT gid, recent_name, recent_clan_tag, highest_score, highest_kills, highest_kd, avg_kd
       FROM tanks_totals
       ORDER BY ${type} DESC
       LIMIT 50`
    )) as RowDataPacket[];
    ...
    for (let i = 0; i < rows.length; i += 10) {
      ...
      let description = "";
      pageRows.forEach((r: { highest_score: any; highest_kills: any; highest_kd: any; avg_kd: any; recent_clan_tag: any; recent_name: any; }, idx: number) => {
        const value = (() => {
          switch (type) {
            case "highest_score": return Number(r.highest_score).toLocaleString();
            case "highest_kills": return Number(r.highest_kills).toLocaleString();
            case "highest_kd": return Number(r.highest_kd).toFixed(2);
            case "avg_kd": return Number(r.avg_kd).toFixed(2);
          }
        })();
        const name = r.recent_clan_tag ? `[${r.recent_clan_tag}] ${r.recent_name}` : r.recent_name;
      ...
      const embed = new EmbedBuilder()
        ...
        .setColor(0x008494)
        .setFooter({ text: `Page ${Math.floor(i / 10) + 1}/${Math.ceil(rows.length / 10)}` });
    ...
    collector.on("collect", async btn => {
      ...
      if (btn.user.id !== interaction.user.id) {
        return btn.reply({ content: "You cannot control this leaderboard.", ephemeral: true });
      }
