  ...
  export async function execute(interaction: ChatInputCommandInteraction) {
    ...
    try {
      ...
      for (const r of rows as any[]) {
        ...
        if (kd > d.highest_kd) d.highest_kd = kd;
      ...
      for (const gid of Object.keys(totals)) {
        const t = totals[gid];
        const avg_kd = t.kd_values.length > 0 ? t.kd_values.reduce((a: number, b: number) => a + b, 0) / t.kd_values.length : 0;
      ...
      for (const key of Object.keys(daily)) {
        const d = daily[key];
        const avg_kd = d.kd_values.length > 0 ? d.kd_values.reduce((a: number, b: number) => a + b, 0) / d.kd_values.length : 0;
