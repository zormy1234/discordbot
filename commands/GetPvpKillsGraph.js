...
export const data = new SlashCommandBuilder()
    ...
    .setDescription('View a cumulative kills or average K/D graph for a Ships3D player')
    .addStringOption((option) => option
    .setName('mode')
    .setDescription('Graph mode')
    .setRequired(true)
    .addChoices({ name: 'Cumulative Kills', value: 'cumulative' }, { name: 'Average K/D', value: 'avg_kd' }))
    .addStringOption((option) => option
    .setName('name')
...
export async function execute(interaction) {
    ...
    try {
        ...
        `;
            ...
            const [dailyRows] = await connection.execute(query, params);
            if (!dailyRows.length) {
                ...
                options: {
                    scales: {
                        ...
                        y: {
                            title: {
                                display: true,
                                text: mode === 'cumulative' ? 'Cumulative Kills' : 'Average K/D',
