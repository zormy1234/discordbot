...
export async function execute(interaction) {
    ...
    await enqueuePrivateDb('redcoats-assignroles', async () => {
        ...
        const rules = await db.execute(`
          ...
          `, [interaction.guildId]);
        if (!rules.length) {
            await interaction.editReply('No role rules configured');
            return;
        }
        let updated = 0;
        for (const link of links) {
            ...
            for (const rule of rules) {
                ...
                if (value >= rule.threshold) {
                    await member.roles.add(rule.discord_role_id).catch(() => { });
                }
            }
            updated++;
        }
        await interaction.editReply(`Processed ${updated} users`);
    });
}
