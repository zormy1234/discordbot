...
export async function execute(interaction) {
    ...
    try {
        ...
        const [rows] = (await connection.execute('SELECT total_kills, total_deaths, recent_name, recent_clan_tag, total_score, total_rank, num_entries, highest_score FROM tanks_totals WHERE gid = ?', [gid]));
        if (!rows.length) {
        ...
        const embed = new EmbedBuilder()
            .setTitle(`📊 Stats for ${stats.recent_clan_tag} | ${stats.recent_name || 'Unknown'} (${gid})`)
            .setColor(0x00ff00)
            .addFields({
            name: 'Total Kills',
            value: stats.total_kills.toString(),
            inline: true,
        }, {
            name: 'Total Deaths',
            value: stats.total_deaths.toString(),
            inline: true,
        }, {
            name: 'Average Score',
