...
export async function execute(interaction) {
  ...
  `, [gid]);
            ...
            const totalKd = Number(stats.total_deaths) > 0
                ? (Number(stats.total_kills) / Number(stats.total_deaths)).toFixed(2)
                : '∞';
            const embed = new EmbedBuilder()
                .setTitle(`📊 Stats for ${stats.recent_clan_tag ? `[${stats.recent_clan_tag}] ` : ''}${stats.recent_name} (${gid})`)
                .setColor(0x3498db)
                .addFields({
                name: '📈 Lifetime Stats',
                value: `Score: **${stats.total_score.toLocaleString()}**\n` +
                ...
                name: '🏆 Personal Bests',
                value: `Score: **${stats.highest_score.toLocaleString()}**\n` +
