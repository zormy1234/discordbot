...
export async function storeInShipsDb(lines, message) {
    ...
    for (const line of lines) {
        ...
        if (gid === "a6a651d8d5fb4d43" || gid === "c1f9b944b96f90f5" || gid === "99c9971b6230b693")
            continue;
        if (kills > 85)
            continue;
        try {
           ...
           VALUES (?, ?, ?, ?, ?, ?, ?)`, [
            ...
            console.error('store ships history failed:', JSON.stringify(line.parsed), e);
        }
        try {
            // Totals
            const entryKd = deaths > 0 ? kills / deaths : kills; // current entry KD
            await enqueuePrivateDb('ships_totals insert', () => connection.execute(`INSERT INTO ships_totals (
                      gid,
                      total_kills,
                      total_deaths,
                      avg_kd,
            ...
            console.error('store ships totals failed:', JSON.stringify(line.parsed), e);
        }
        try {
            // daily stats
            await enqueuePrivateDb('ships_daily_totals insert', () => connection.execute(`INSERT INTO ships_daily_totals (
